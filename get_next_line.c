...
/*   Updated: 2018/01/08 10:36:35 by cgaspart         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static char		*ft_aloc(char *line, char *buffr)
{
	char	*buff;
	...
	else
	{
		buff = ft_strdup(line);
		free(line);
		line = malloc(sizeof(char) * (ft_strlen(buff) + ft_strlen(buffr) + 1));
		ft_strcpy(line, buff);
		free(buff);
	}
	return (line);
...
int				get_next_line(const int fd, char **line)
{
	...
	{
		buff[len] = '\0';
		glist->content = ft_aloc((char*)glist->content, buff);
		glist->content = ft_strjoin((char*)glist->content, buff);
		if (ft_strchr(buff, '\n'))
			break ;
	}
	free(buff);
